...
def is_conflicting_update(device, build_type, build_id) -> bool:
    result = db.session.query(Update).filter(Update.device == device,
...
def update_get(device: str, buildtype: str, build_id: str):
    ...
    current_version = db.session().query(Update).filter_by(build_id=build_id, device=device,
